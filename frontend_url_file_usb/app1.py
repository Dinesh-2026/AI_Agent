...
# ================== CONFIG ==================
API_KEY = "YOUR_NEW_API_KEY"   # ⚠️ REPLACE THIS

DEMO_MODE = False   # Set True for college demo screenshots

# ============ VirusTotal Endpoints ============
VT_URL_SCAN_SUBMIT = "https://www.virustotal.com/vtapi/v2/url/scan"
...
def check_url_safety(url):
    ...
    # ---- DEMO MODE ----
    if DEMO_MODE:
        return "❌ UNSAFE — 3/92 engines detected this URL (Demo Mode)"

    # Step 1 — Submit URL
    submit_params = {'apikey': API_KEY, 'url': url}
    submit_result = safe_vt_request(VT_URL_SCAN_SUBMIT, params=submit_params)

    if "error" in submit_result:
        return f"⚠️ VirusTotal Submit Error: {submit_result['error']}"
    ...
    report = safe_vt_request(VT_URL_REPORT, params=report_params)

    if "error" in report:
        return f"⚠️ VirusTotal Report Error: {report['error']}"

    positives = report.get("positives")
    total = report.get("total")

    if positives is None:
        return "⏳ Scan submitted. Report not ready yet. Try again."

    if positives > 0:
        return f"❌ UNSAFE — {positives}/{total} engines detected this URL"
    else:
        return f"✅ SAFE — {positives}/{total} detections"

# ================== FILE SCAN ==================
def check_file_safety(file_path):

    if DEMO_MODE:
        return "❌ UNSAFE — 2/70 engines detected this file (Demo Mode)"
    ...
    for partition in psutil.disk_partitions():
        if 'removable' in partition.opts or 'cdrom' in partition.opts:
            try:
                ...
                for f in files:
                    if f.lower() == "autorun.inf" or f.endswith((".lnk", ".exe")):
